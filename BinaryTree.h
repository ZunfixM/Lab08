...
private:
   ...
   // Write the values stored in the tree accessed by the subtree.
...
void BinaryTree:: buildRandom( long size, BinaryNode * & subtree )
{
   if( size == 0 )
   {
      subtree = NULL;
...
// Use a line by line display, order nodes from left to
//   right, draw connecting lines.
void BinaryTree:: displayLeft( std::ostream & outfile, BinaryNode * subtree, std::string prefix )
{
   if( subtree == NULL )
   {
...
long BinaryTree:: height( const BinaryNode * subtree )
{
	...
	else
	{
		...
		int rHeight = height(subtree->right_);
